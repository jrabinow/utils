...
StringBuffer append_to_stringbuffer(StringBuffer sb, char *str) __attribute__ ((nonnull (2)));

/* Convert from string buffer to string. Free return value when done */
char *stringbuffer_to_string(StringBuffer sb) __attribute__ ((nonnull));
...
#  define __SIZEOF_ARRAY_STRUCT	(offsetof(struct __array_data__, type))
# else
...
int init_log(FILE *stream, log_level_t loglevel) __attribute__ ((nonnull));
